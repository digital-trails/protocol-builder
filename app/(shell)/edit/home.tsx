...
export default function EditHomeScreen() {
  ...
  function homeDispatch(home: any, action: any) {
    ...
    if (action.type === 'title') home.title = action.value;
    if (action.type === 'flavor0') home.flavors[0] = action.value;
    if (action.type === 'flavor1') home.flavors[1] = action.value;
    if (action.type === 'sessions') home.sessions = !home.sessions;

    if (action.type === 'label0') home.buttons[0][0] = action.value;
    if (action.type === 'label1') home.buttons[1][0] = action.value;
    if (action.type === 'label2') home.buttons[2][0] = action.value;
    if (action.type === 'label3') home.buttons[3][0] = action.value;

    if (action.type === 'icon0') home.buttons[0][1] = action.value;
    if (action.type === 'icon1') home.buttons[1][1] = action.value;
    if (action.type === 'icon2') home.buttons[2][1] = action.value;
    if (action.type === 'icon3') home.buttons[3][1] = action.value;
  ...
  return (
    <PhoneSplit state={home} screen='home'>
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'flex-end' }}>
        ...
        <View style={styles.row}>
          <View style={styles.input}>
            <Input label='Sessions' type='switch' value={home.sessions} onValueChange={() => homeDispatch(home, { type: 'sessions' })} />
          </View>
          <View style={styles.input} />
          <View style={styles.input} />
        </View>
